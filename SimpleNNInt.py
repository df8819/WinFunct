...
class SimpleNN:
    ...
    def predict(self):
        ...
        progress_callback = SimpleNN.ProgressCallback()
        progress_callback.set_simple_nn_instance(self)  # Set the SimpleNN instance
    ...
    class ProgressCallback(keras.callbacks.Callback):
        def __init__(self):
            super().__init__()
            self.progress = 0
            self.simple_nn_instance = None  # This should refer to the SimpleNN instance

        def on_epoch_end(self, epoch, logs=None):
            if self.simple_nn_instance:  # Make sure it is set before using it
                self.progress += 1
                self.simple_nn_instance.progress_bar["value"] = self.progress
                self.simple_nn_instance.progress_bar.update()

        def set_simple_nn_instance(self, simple_nn_instance):
            self.simple_nn_instance = simple_nn_instance  # Setter method"""